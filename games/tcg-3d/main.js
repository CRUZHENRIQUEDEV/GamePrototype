...
async function init() {
  ...
  scene.addLight('directional', { position: [5, 12, 8], intensity: 1.2, shadows: true });
  scene.addLight('point', { position: [0, 6, 0], color: 0x4466ff, intensity: 0.3 });
  ...
  const lineMat = new THREE.LineBasicMaterial({ color: 0x4466cc, opacity: 0.5, transparent: true });
...
function startGame(cardDb) {
  ...
  engine.initGame([localId, opponentId], { [localId]: deck, [opponentId]: deck });

  bus.on('card:drawn',    () => render3DHand());
  bus.on('card:played',   d  => { render3DField(d.target); render3DHand(); });
...
function buildDeck(cardDb) {
  return Object.values(cardDb).flatMap((c, i) => [
...
function createCardMesh(card, THREE) {
  ...
  const colors = { creature: 0x1b4332, spell: 0x1d3557, artifact: 0x3d2b1f, trap: 0x4a0e4e };
...
function setupRaycaster() {
  ...
  input.bind('Space', () => {
    ...
    if (hits.length) {
      ...
      const ok = engine.applyAction({ type: 'play-card', cardId: card.instanceId }, localId);
      if (ok) {
