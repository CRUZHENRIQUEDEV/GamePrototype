...
export class CheckersEngine {
  ...
  selectPiece(row, col) {
    ...
    if (mustCaptureFrom &&
        (mustCaptureFrom.row !== row || mustCaptureFrom.col !== col)) return false;
  ...
  move(toRow, toCol) {
    ...
    if (!move) return null;
    return this._doMove(selected.row, selected.col, toRow, toCol, move.captures);
  }
  ...
  applyRemoteMove(fromRow, fromCol, toRow, toCol) {
    ...
    const captures = (this._getCaptures(fromRow, fromCol, piece, board)
      .find(m => m.row === toRow && m.col === toCol)?.captures) ?? [];
  ...
  _doMove(fr, fc, tr, tc, captures) {
    ...
    if (captures.length > 0 && !promoted) {
      ...
      if (chain.length > 0) {
        ...
        return { success: true, chainCapture: true, captured, promoted,
  ...
  _getMovesForPiece(row, col) {
    ...
    // Se há captura forçada em qualquer peça do turno, só capturas são válidas
    if (this._anyForcedCapture(turn, board)) return captures;
    return captures.length > 0 ? captures : this._getSimpleMoves(row, col, piece, board);
  }
  ...
  _getCaptures(row, col, piece, board) {
    ...
    for (const [dr, dc] of [[-1,-1],[-1,1],[1,-1],[1,1]]) {
      const mr = row + dr,     mc = col + dc;      // casa do inimigo
      const nr = row + dr * 2, nc = col + dc * 2;  // destino
  ...
  _hasAnyMove(color) {
    ...
    for (let r = 0; r < 8; r++)
      for (let c = 0; c < 8; c++)
        if (board[r][c]?.color === color) {
          if (this._getCaptures(r, c, board[r][c], board).length > 0)   return true;
          if (this._getSimpleMoves(r, c, board[r][c], board).length > 0) return true;
        }
    return false;
  }

  _inBounds(r, c) { return r >= 0 && r < 8 && c >= 0 && c < 8; }
  _emptyBoard()   { return Array.from({ length: 8 }, () => Array(8).fill(null)); }
}
