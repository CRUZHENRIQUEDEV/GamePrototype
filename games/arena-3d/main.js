...
function buildArena() {
  ...
  OBSTACLES.forEach(([x, z]) => {
    const pillar = new THREE.Mesh(
      new THREE.BoxGeometry(1.5, 2.0, 1.5),
      pillarMat
    );
    ...
    switch (type) {
      case "sys:join-req": // Host recebe pedido de entrada
        if (isHost) {
          ...
          players.forEach((p) =>
            currentPlayers.push({ id: p.id, idx: getPlayerIndex(p.id) })
          );
...
function setupLobbyUI(roomId, amHost) {
  ...
  if (amHost) {
    ...
    document.getElementById("lobby-status").textContent =
      "Você é o Host. Aguarde jogadores.";
  } else {
...
function updateLobbyList() {
  ...
  players.forEach((p) => {
    ...
    div.innerHTML = `<span>${p.id} ${
      p.id === myId ? "(Você)" : ""
    }</span> <span>Pronto</span>`;
...
function startGame() {
  ...
  document.getElementById("btn-fps").style.display = "block";
  ...
  players.forEach((p) => {
    ...
    div.innerHTML = `
      ...
      <div class="hp-bar-bg"><div id="hp-bar-${p.id}" class="hp-bar ${
      p.id === myId ? "local" : "remote"
    }" style="width:100%; background:${
      "#" + p._baseColor.toString(16)
    }"></div></div>
...
function addPlayer(id, spawnIndex, isLocal) {
  ...
  const p = new Player(id, scene.scene, scene._THREE, {
    x: spawn.x,
    z: spawn.z,
    color: spawn.color,
    isLocal: isLocal,
...
function updateProjectiles(delta, THREE) {
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    if (
      p.mesh.position.y < 0 ||
      Math.abs(p.mesh.position.x) > 10 ||
      Math.abs(p.mesh.position.z) > 6 ||
      p.lifetime <= 0
    ) {
...
function updateBot(delta, THREE) {
  ...
  if (dist > 4) {
    ...
    const dir = new THREE.Vector3()
      .subVectors(player.group.position, bot.group.position)
      .normalize();
    bot.move(dir, delta);
  } else {
    // Strafe
...
function enableFPS() {
  ...
  setTimeout(
    () => document.getElementById("fps-lock-hint").classList.remove("visible"),
    2500
  );
}
